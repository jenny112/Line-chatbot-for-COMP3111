...
public class SQLDatabaseEngine {
			...
			if (tourName.toLowerCase().contains(text.toLowerCase()) || tourDescription.toLowerCase().contains(text.toLowerCase())
				...
				while (rs2.next()) {
					...
					if(availableCapacity == 0) {
						...
						tourArr.get(index).setCapacity(0);
					}
					else {
						tourAvailableDate.add(startDate);
	...
	ArrayList<String[]> searchBookingRecord(String id) throws Exception {
		...
		Connection connection = getConnection();
		PreparedStatement stmt = connection.prepareStatement("SELECT T.TourName, B.* FROM BookingRecord B, Tour T, Customer C WHERE T.TID = B.TID AND C.CID = B.CID AND C.LineID = '" + id + "'" );
		ResultSet rs = stmt.executeQuery();
		while (rs.next()) {
